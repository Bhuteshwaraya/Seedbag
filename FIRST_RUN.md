...
## Keep this device ready for future projects

Start with the existing host instructions and a short check of the required executables, authorized account connection, and ordinary Git access to the selected repository. Reuse a healthy route and proceed to the project. Do not repeat installation, sign-in, or transport investigation after those checks pass. Investigate only the failing layer. Avoid a task-specific `GH_CONFIG_DIR`: it can hide the person's working shared CLI configuration and create another setup to maintain.

Before establishing durable credentials, distinguish the intended account owner from the process running the command. Native Codex on Windows can execute through dedicated lower-privilege sandbox users. A temporary sandbox identity is not automatically the right owner for the person's reusable credentials. When existing credentials are inaccessible, check whether the host provides an approved execution route as their owner before proposing a permissions repair. This does not authorize elevation or bypassing sandbox policy. See the [Windows sandbox documentation](https://learn.chatgpt.com/docs/windows/windows-sandbox).

After a successful setup, retain a small, non-secret device note in the host's existing assistant-wide instructions when writing there is permitted. For Codex, use the active global instruction file under the configured `CODEX_HOME` (normally `~/.codex`); preserve existing content and honor `AGENTS.override.md` and other overrides. Do not add a competing instructions system. Record only the verified account/provider, Git transport and credential-manager name, supported execution route, relevant tool versions, verification date, and the commit identifier of any authorized private checkpoint whose publication and remote readback succeeded. Keep device paths and configuration details local; credentials and key contents belong in neither notes nor project files. [Codex instruction discovery](https://learn.chatgpt.com/docs/agent-configuration/agents-md) explains the global scope.

That note saves rediscovery; it is neither current proof of access nor permission for future privileged operations. On another device, establish and verify that device's own route. On this device, use fresh narrow checks before relying on the saved route. An authenticated private read and an already-authorized publication with exact remote readback establish the tested private workflow; a public repository read alone does not. Do not create a test repository or push unrelated changes just to refresh the note.
...
The helper's `recommended_actions` selects fixed guidance from observed results. It distinguishes helper launch failures, untrusted repository ownership, SSH host verification, malformed keys, inaccessible keys, API rejection, and network faults. Use those recommendations to investigate the failing layer; they do not install tools, repair permissions, or authorize changes. Offline results make no authentication or network recommendation.
...
After installation or a persistent environment change, verify discovery and the connection from a fresh execution context representative of a later task. A one-command PATH change does not prove future readiness. Inspect applicable [Codex configuration precedence](https://learn.chatgpt.com/docs/config-file/config-basic) and [shell environment policy](https://learn.chatgpt.com/docs/config-file/config-reference) if values differ. Windows child processes normally inherit their parent's environment; restart the application only when evidence shows its environment is stale, and coordinate any restart with active work. See [Windows environment inheritance](https://learn.microsoft.com/en-us/windows/win32/procthread/environment-variables).
...
For an ownership error, verify the actual repository and execution owner. Never apply broad ACL changes or wildcard `safe.directory` entries; any permitted trust exception must name the reviewed repository. [Git's ownership checks](https://git-scm.com/docs/git-config#Documentation/git-config.txt-safedirectory) protect repositories owned by another user. Consider [SSH over port 443](https://docs.github.com/en/authentication/troubleshooting-ssh/using-ssh-over-the-https-port) only for a diagnosed transport need and a host-approved route. Verify GitHub's published host-key fingerprint before accepting a new host key. This is an optional transport choice, not a universal setup step.
