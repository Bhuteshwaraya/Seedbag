...
`README.md` is the planted project's recovery page. It links goals, progress, and the short request in `CONTINUE_HERE.md`, and directs the assistant to AGENTS.md before interpreting saved state. The person can use ordinary wording; the request locates the files. The installer generates matching marked blocks containing `Continue my project: LOCATOR`. The saved locator must exactly identify the repository recorded in the ledger, or an absolute folder for an explicitly local-only project. `doctor`, the staged gate, and committed audits check required project files and those blocks. Snapshot audits preserve the entire continuation file against ledger-bearing parents. The README prose outside its prompt block may develop with the project. Repository relocation has no supported migration command; do not rewrite the saved entry during ordinary work.
...
A mutation can save the ledger before updating generated views. If that final render fails, the CLI reports a partial save with the mutation result and a fresh ledger observation when readable; it does not report ordinary success. Some views may still be stale. Inspect the recorded revision and original input before retrying, preserve any conflicting view text, and reconcile deliberately. A failed readback remains uncertain. This does not certify that a cloud filesystem will retain state after a later environment change.
