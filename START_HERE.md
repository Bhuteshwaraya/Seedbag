# Assistant bootstrap for Seedbag 0.4.1
...
Also establish how this conversation will load and run the project's synchronization callbacks. Verify its actual project/workspace directory and the host's supported activation route. A shell command's `workdir` does not move the conversation or make it discover callbacks installed in another folder. Do not promise protected project work based on file access or Git access alone.
...
Choose the project folder before downloading the package. Prefer the actual conversation's unused working directory when it is a suitable new project workspace; keep bootstrap downloads outside that directory. If the host created only empty `work` and `outputs` directories there, you may remove those empty directories after checking them so the installer receives an empty destination. Never remove a populated directory or move existing work to make it appear empty. Do not create a sibling project folder merely for a nicer name if that would leave this conversation outside its safeguards. If the current working directory is already populated, preserve it and use a supported route into a new project workspace.
...
Resolve the published v0.4.1 release of [Bhuteshwaraya/Seedbag](https://github.com/Bhuteshwaraya/Seedbag) to one exact commit. Read this guide, FIRST_RUN.md, SYNC.md, OPERATIONS.md, and the manifest from that commit. Retrieve and verify all listed file hashes before executing the package. Retain the release and commit as setup provenance. Do not mix a moving branch's instructions with another release's runtime.
...
If synchronization is temporarily blocked, preserve that input in a complete handoff or an available host-permitted recovery location; active hooks may deny arbitrary scratch-file writes. Do not invent a successful capture. If only setup and a name were requested, record that the goal is unspecified. Ask what the project should achieve only after setup is verified complete. Do not infer an implementation from a project name.
...
On a supported Codex host, prepare callbacks with `install-host-hooks` after connection setup. Inspect generated commands and follow the actual host trust control. Verify that the host discovers and trusts these exact definitions for the actual project directory, and observe the host dispatching a `PreToolUse` callback for a covered tool in this conversation. A generated file, a callback invoked manually, or a test in another conversation does not prove this conversation is protected. Do not install global callbacks or use a trust-bypass flag to conceal missing project activation. The supported host trust control may retain the reviewed definitions' trust in the host's own configuration. Keep this machine's ignored hook configuration local. The separate local Git snapshot hook is installed at planting; preserve existing hooks during integration. See SYNC.md for exact boundaries.

A saved private checkpoint and readiness for protected project work are separate results. Until the actual host activation and tool interception are verified, setup is incomplete and new product work stays paused. You may preserve the opening brief and finish the private setup checkpoint through the guarded runtime while activation is pending. Before handing off, verify that checkpoint and release the writer so the next conversation can finish setup; if either fails, retain and report the exact unresolved state. A cloud or other host with runtime-command gates alone cannot complete the automatic editing protection route.

For a supported Codex installation, the project's `configure-codex` helper inspects actual registration. After reviewing its verified code and generated commands, use `configure-codex --trust-reviewed` to retain narrowly scoped trust through the host API. This authorized setup does not require a second permission ritual. The helper preserves unrelated settings and never treats trusted configuration as proof that an already running task has loaded it. Follow SYNC.md and verify actual host execution in the project conversation before calling it ready.

If this conversation cannot activate the callbacks, use a verified supported route into the saved project's directory. Give one unavoidable plain-language action if the person must open or approve something, with a complete setup handoff preserving their brief and the existing repository. Do not invent a host control, create a replacement project, or invite product work while activation remains unverified.

Report the verified working location, private repository, and observed synchronization/host-hook status. Say setup is complete only when private preservation and this conversation's automatic editing checks are both verified. Otherwise say what is saved and that setup still needs completion; provide the next action or handoff instead of asking to begin product work. For cloud creation, state that a folder on the person's computer has not yet been created. Return the short request from CONTINUE_HERE.md unchanged; it locates saved work and does not certify this device's readiness. Explain that the same request later lets a capable local conversation retrieve the same repository. A new conversation must never create a replacement repository or fetch a newer seed to resume.
